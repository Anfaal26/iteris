...
- **HEAD:** `c2d656b` on `main`, in sync with `origin/main`.
...
- **`notebooks/00_free_diagnostics_all_classes.ipynb`** (added 2026-06-26) — run this before any GPU training round. Warm-starts the lite U-Net + runs Pillar 4 + headroom diagnostics for every CAMUS class and BRISC generic-tumour by default; BRISC subtypes gated behind a flag (each costs a full ~8–15 min U-Net pass). Outputs a per-class go/no-go verdict + `/kaggle/working/diagnostics_summary.csv`.
