...
### CI/CD — pinned GitHub Actions versions & Node.js runtime deprecation
- **GitHub periodically retires the Node.js runtime an Action's JS code runs on, and each first-party Action publishes a new major version to move to it — pinning an old major (e.g. `@v4`) doesn't break the workflow immediately, but shows up as a yellow "Node.js X is deprecated... being forced to run on Node.js Y" warning on every run once the old runtime is past end-of-life. This is a warning, not (yet) a failure, so it's easy to ignore for months until GitHub actually removes the old runtime and the run starts failing outright.**
- **The fix is always the same shape: bump the pinned major version(s) of the offending Action(s), not the Node version itself** — you don't control the Node runtime directly, only which Action major (which bundles a specific Node target) you pin.
- **Confirmed version→runtime jumps (verified 2026-08-01 against each Action's own GitHub releases, don't trust memory/training data on this — check the releases page, since these change over time):** `actions/checkout` moved to Node 24 at **v5.0.0** (latest at the time: v7); `actions/setup-python` moved to Node 24 at **v6.0.0** (latest: v7); `actions/setup-node` moved to Node 24 at **v5.0.0** (latest: v7). All three were still pinned at their old (Node 20) majors (`@v4`/`@v5`/`@v4` respectively) across `.github/workflows/*.yml` until this was caught — bumped to latest majors across `ci-python.yml`, `ci-ui.yml`, `deploy-hf-space.yml`.
- **`ubuntu-latest`/other GitHub-hosted runners always have a current-enough runner version** for whatever Node bump a new Action major needs (self-hosted runners are the ones that can fall behind and need a `runner v2.327.1+`-style minimum called out in the Action's own release notes — not a concern here, but check it if a workflow ever moves to self-hosted).
- **When adding or touching any workflow file, glance at whether its pinned Action majors are current** — a few-line version bump is free; catching it before GitHub hard-removes the old runtime is what avoids a workflow silently going red with no code change to point to.
